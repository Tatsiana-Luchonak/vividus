...
import static org.vividus.selenium.mobileapp.screenshot.util.CoordsUtil.scale;
...
@Named("FULL")
public class MobileAppFullShootingStrategyFactory implements ScreenshotShootingStrategy
{
    private static final Logger LOGGER = LoggerFactory.getLogger(MobileAppFullShootingStrategyFactory.class);
    ...
    public MobileAppFullShootingStrategyFactory(TouchActions touchActions,
    ...
    {
        return new MobileAppFullShootingStrategy();
    }

    public class MobileAppFullShootingStrategy implements ShootingStrategy
    {
        private static final long serialVersionUID = 2364824713307149833L;
        ...
        private BufferedImage concatImages(List<BufferedImage> images)
        {
        ...
        private BufferedImage concatByTemplate(BufferedImage first, BufferedImage second, BufferedImage template)
        ...
        private BufferedImage cropImage(BufferedImage image, int fromY, int toY)
        {
            return image.getSubimage(0, fromY, image.getWidth(), toY);
        }

        private int getOverlapY(BufferedImage image, BufferedImage template) throws TemplateMissmatchException
        {
        ...
        private DogArray<Match> match(GrayF32 image, GrayF32 template)
        {
            TemplateMatching<GrayF32> matcher = FactoryTemplateMatching.createMatcher(TemplateScoreType.NCC,
                    GrayF32.class);
        ...
        private IntStream asInts(List<BufferedImage> images, ToIntFunction<BufferedImage> function)
        {
